...
For the potentiometer, a simple division algorithm was used to divide the ADC output from a number between 0 and 1023, to a number between 0 and a value dependant on each menu. This splits the input into a number of even sections to be used by the Menu System.
...
\section{Module: Usability Software (EEPROM)}
\subsection{Description}    
An EEPROM read/write module was added to the system to increase user
friendliness and system deploy-ability. EEPROM allows for values to be saved in a non-volatile memory location, such that when power cycled, the system will retain important calibration settings. This reduces the maintenance required on the system, and limits the needed interactions of technically trained users to change functional
...
This module was implemented through two public subroutines, sendEep and readEep.
SendEep takes an integer number and an integer address, stores the data in two consecutive memory bytes.
readEep takes an integer address, reads the value at that address into two characters, concatenates the number and returns an integer value.

\chapter{User Interface Design}
%Give a detailed description of the design of the user interface. This will gave the reader a good view of how the system functions from the user's perspective.
...
\section{Interface Design: End User}
...
\section{Interface Design: Technical User}
...
\begin{figure}
\centering
\includegraphics[width=0.7\linewidth]{"../Diagrams/MenuSystem"}
\caption[Menu Diagram]{Menu Diagram}
\label{fig:MenuStructureDiagram}
\end{figure}
...
\subsubsection{Temperature (LM35)}
The LM35 is a centigrade temperature sensor calibrated directly in Degrees Celsius. It’s also has a linear + 10 mV/°C scale factor. Only a basic configuration of the temperature sensor was used as we only need to measure between to +2°C to +150°C as shown in the figure below. We found that 0mv started at +2°C so we had to adjust the final calculations by -20mV offset to get accurate temperature readings.

\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{"../Diagrams/tempResponse"}
\caption[LM35]{Temperature sensor linear response and wiring diagram}
\label{fig:tempResponse}
\end{figure}

\subsubsection{IR [Distance Measuring] (GP2Y0A02YK0F)}
...
Decimal Value = (1024 / 5Vref) * (Output Voltage at given distance)
\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{"../Diagrams/IRresponse"}
\caption[LM35]{Infrared sensor response}
\label{fig:IRresponse}
\end{figure}

\subsubsection{Ultrasonic (6500)}
This sonar ranging sensor is able to measure distances from 0.15m to 10.67m with a typical 1% error.
We used single-echo mode in our system as we were only tracking one object and therefore only need one echo output at a time. Basically the distance between to a target is measured by the time between INIT going high and ECHO going high as seen in the figure below. The signal travels at approximately 0.274 milliseconds per meter.

\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{"../Diagrams/ultrasonicResponse"}
\caption[LM35]{Ultrasonic sensor response and timing diagram}
\label{fig:ultrasonicResponse}
\end{figure}
...
\subsection{CircularBuffers.h}
\lstinputlisting{../Code/CircularBuffers.h}
\subsection{Common.h}
\lstinputlisting{../Code/Common.h}
\subsection{EEPROM.c}
\lstinputlisting{../Code/EEPROM.c}
\subsection{Interrupts.c}
\lstinputlisting{../Code/Interrupts.c}
\subsection{jEEP.h}
\lstinputlisting{../Code/jEEP.h}
\subsection{LCD.h}
\lstinputlisting{../Code/LCD.h}
\subsection{LCD\_defs.h}
\lstinputlisting{../Code/LCD_defs.h}
\subsection{LCD.c}
\lstinputlisting{../Code/LCD.c}
\subsection{MenuDefs.h}
\lstinputlisting{../Code/MenuDefs.h}

\subsection{Menusystem.h}
\lstinputlisting{../Code/Menusystem.h}

\subsection{Menusystem.c}
\lstinputlisting{../Code/Menusystem.c}

\subsection{newmain.c}
\lstinputlisting{../Code/newmain.c}

\subsection{PanTilt.h}
\lstinputlisting{../Code/PanTilt.h}
\subsection{PanTilt.c}
\lstinputlisting{../Code/PanTilt.c}
\subsection{Range.h}
\lstinputlisting{../Code/Range.h}
\subsection{Range.c}
\lstinputlisting{../Code/Range.c}
\subsection{Serial.h}
\lstinputlisting{../Code/Serial.h}
\subsection{Serial.c}
\lstinputlisting{../Code/Serial.c}
\subsection{Temp.h}
\lstinputlisting{../Code/Temp.h}
\subsection{Temp.c}
\lstinputlisting{../Code/Temp.c}
\subsection{Tracking.h}
\lstinputlisting{../Code/Tracking.h}
\subsection{Tracking.c}
\lstinputlisting{../Code/Tracking.c}
\subsection{User\_Interface.h}
\lstinputlisting{../Code/User_Interface.h}
\subsection{User\_Interface.c}
\lstinputlisting{../Code/User_Interface.c}
