...
\begin{abstract}
	...
	IMPORTANT NOTE: This system can run on either a minimal board, or a PICDEM board by defining or not defining MNML in Common.h. If MNML is not defined when run on the minimal board, it will damage the servos (PDM will be 125Hz). If MNML is defined the system will not run on a PICDEM board.
\end{abstract}
...
\section{Description}
The Yavin IV Death Star Tracker includes the following hardware:

\begin{itemize}
	\item Ultrasonic Sensor
	\item IR Sensor
	\item Pan Tilt Base mechanism
	\item LCD display
\end{itemize}

A further description of the system hardware is included in the hardware section.

\section{Configuration/Layout}
there are two main parts to the Yavin IV Death Star Tracker; the Pan Tilt module which contains all the sensors and actuators and the control panel which contains all the user interface hardware (buttons LCD etc.). \newline
Below is an exploded view of the component layout within the control panel.
...
\begin{itemize}
	...
	\item Include PanTilt.h within the file you wish to use the module
	\item Call configureBase() to set up the module
...
\chapter{Minimal Board}
\section{Description}
The minimal board is a PCB on which is mounted the micro controller running the system, power regulators, IO pins, and the serial port.

\section{Power Requirements}
Requires a 5v regulated power supply, or a 9v unregulated supply. The current draw is dependant on the computations being performed and other factors.

\chapter{Ultrasonic Sensor}
\section{Description}
The ultrasonic sensor is a polarised sonar sensor which sends out an audible click and detects the return of the reflected tick (actually a series of pulses). The range to an object is calculated based on the flight time of the return signal.

\section{Power Requirements}
The ultrasonic sensor requires a regulated 5v power supply.  The sensor primarily draws current when it begins pulsing, and can draw up to 2A during this short time. For this reason the ultrasonic sensor has a 1mF bypass capacitor and its own regulated bus. This means these sharp current spikes do not affect the voltage to the other components.
...
\section{Public Interface functions undeclared}
\subsection{Description}
Whenever you try to use the documented public interface functions the compiler tells you that they are undeclared.
...
\subsection{Source}
Another possible cause is that the string you are passing is not correctly null terminated. In this case you should expect to see some number (up to BUFFERLENGTH) characters transmitted which may or may not include the desired string.

\section{Incorrect Calculations}
\subsection{Description}
The processor is returning incorrect results to simple arithmetic calculations such as multiplication.

\subsection{Cause}
A very common issue in MPLabX is that the complier does not support integer promotion. This means if you have a literal times something else, e.g. 32*17 you may need to cast them into a larger integer type to perform the calculation, or the result may overflow. e.g. (unsigned long)32*(unsigned long)17.
