...
void increment(Direction difference)
{
    ...
    current_direction.elevation += difference.elevation;
...
void incrementFine(Direction difference)
{
    ...
    inc = global_delay.InclinationDelay + difference.elevation + LATENCY;
...
void calibratePanTiltRange(Direction reference)
{
    ...
    arcRange.elevation = arcRange.elevation * reference.elevation / current_direction.elevation;
    current_direction = reference;
}
...
static Direction delay2Direction(Delay dly)
{
    ...
    ret.elevation = ((dly.InclinationDelay - DUTY_CYCLE_TIME + LATENCY) * (long int)arcRange.elevation + DIV_2(DUTY_CYCLE_TIME))/ DUTY_CYCLE_TIME;

    ret.azimuth = ret.azimuth - DIV_2(arcRange.azimuth) - calibration_offset.azimuth;
    ret.elevation = -(ret.elevation - DIV_2(arcRange.elevation) - calibration_offset.elevation);
...
static Delay direction2Delay(Direction dir)
{
    ...
    inc = DUTY_CYCLE_TIME + (-dir.elevation + DIV_2(arcRange.elevation) + calibration_offset.elevation) * (long int)DUTY_CYCLE_TIME / arcRange.elevation;
